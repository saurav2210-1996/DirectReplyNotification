...
import com.codility.call_record.CrLog;
...
class RecordingThreadWav extends RecordingThread implements Runnable {
    ...
    public void run() {
        try (FileOutputStream outputStream = new
            ...
            //CrLog.log(com.codility.call_record.CrLog.ERROR, "Error while writing temp pcm file: " + e.getMessage());
            if(!tmpFile.delete())
                //CrLog.log(com.codility.call_record.CrLog.ERROR, "Cannot delete incomplete temp pcm file.");
    ...
    static class CopyPcmToWav implements Runnable {
        ...
        public void run() {
            ...
            try (FileInputStream tmpInput = new FileInputStream(tmpFile);
                ...
                //CrLog.log(com.codility.call_record.CrLog.ERROR, "Error while copying temp pcm to wav file: " + e.getMessage());
                if(!tmpFile.delete() )
                    //CrLog.log(com.codility.call_record.CrLog.ERROR, "Error while deleting temp pcm file on exception.");
                if(!wavFile.delete())
                    //CrLog.log(com.codility.call_record.CrLog.ERROR, "Error while deleting wav file on exception.");
                recorder.setHasError();
                notifyOnError(context);
            }
            finally {
